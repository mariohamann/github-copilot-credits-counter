(function () {
  ...
  var state = { dateFrom: null, dateTo: null, selectedProjects: new Set() };
  var charts = {};
  ...
  function buildKpis(grandTotal, projectCount, requestCount, promptTokens, outputTokens) {
    ...
    return '<div class="kpi-grid">' +
      card('Total Credits', fmt(grandTotal), projectCount + ' project' + (projectCount !== 1 ? 's' : '')) +
  ...
  function buildChartSlots() {
    return '<div class="charts-grid">' +
      '<div class="chart-card"><h3>Credits by Project</h3><div class="chart-wrap"><canvas id="chart-bar"></canvas></div></div>' +
      '<div class="chart-card"><h3>Credits by Model</h3><div class="chart-wrap"><canvas id="chart-doughnut"></canvas></div></div>' +
      '<div class="chart-card wide"><h3>Credits Over Time</h3><div class="chart-wrap"><canvas id="chart-line"></canvas></div></div>' +
      '</div>';
  }
  ...
  function buildTable(projects) {
    ...
    var rows = projects.map(function (p) {
      ...
      return '<tr><td>' + esc(p.projectName) + '</td><td class="num">' + fmt(p.totalCredits) + '</td><td class="num">' + p.sessions.length + '</td><td class="num">' + fmt(pt) + '</td><td class="num">' + fmt(ot) + '</td><td style="color:var(--text2)">' + esc(models) + '</td></tr>';
    }).join('');
    return '<div class="section"><h2>Projects</h2><div class="table-wrap"><table><thead><tr><th>Project</th><th class="num">Credits</th><th class="num">Sessions</th><th class="num">Prompt Tokens</th><th class="num">Output Tokens</th><th>Models</th></tr></thead><tbody>' + rows + '</tbody></table></div></div>';
  }
    ...
    var items = projects.map(function (p) {
      var sessions = p.sessions.map(function (s) {
        var reqRows = s.requests.map(function (r) {
          return '<tr><td>' + esc(r.model || '—') + '</td><td class="num">' + fmt(r.credits) + '</td><td class="num">' + fmt(r.promptTokens) + '</td><td class="num">' + fmt(r.outputTokens) + '</td><td style="color:var(--text2)">' + fmtDate(r.timestamp) + '</td></tr>';
        }).join('');
        return '<div class="session-header">Session: ' + esc(s.sessionId || s.chatFile) + ' &bull; ' + fmt(s.totalCredits) + ' credits &bull; ' + s.requests.length + ' requests</div>' +
          '<div class="table-wrap"><table><thead><tr><th>Model</th><th class="num">Credits</th><th class="num">Prompt Tokens</th><th class="num">Output Tokens</th><th>Timestamp</th></tr></thead><tbody>' + reqRows + '</tbody></table></div>';
      }).join('');
      return '<div class="accordion"><div class="accordion-header" data-accordion><span>' + esc(p.projectName) + ' <span class="proj-meta">' + fmt(p.totalCredits) + ' credits</span></span><i class="chevron">&#9654;</i></div><div class="accordion-body">' + sessions + '</div></div>';
    }).join('');
  ...
  function attachEvents() {
    ...
    if (resetEl) resetEl.addEventListener('click', function () { state = { dateFrom: null, dateTo: null, selectedProjects: new Set() }; render(); });
  ...
  function renderCharts(projects, requests) {
    ...
    if (barCtx && projects.length) {
      charts['bar'] = new Chart(barCtx, {
        ...
        data: {
          ...
          datasets: [{ label: 'Credits', data: projects.map(function (p) { return p.totalCredits; }), backgroundColor: projects.map(function (_, i) { return PALETTE[i % PALETTE.length] + 'cc'; }), borderColor: projects.map(function (_, i) { return PALETTE[i % PALETTE.length]; }), borderWidth: 1, borderRadius: 4 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { label: function (c) { return ' ' + fmt(c.parsed.y) + ' credits'; } } } }, scales: { x: { ticks: { color: textColor }, grid: { color: gridColor } }, y: { ticks: { color: textColor }, grid: { color: gridColor }, beginAtZero: true } } }
      });
    }
    ...
    if (doCtx && requests.length) {
      ...
      charts['doughnut'] = new Chart(doCtx, {
        type: 'doughnut',
        data: { labels: sorted.map(function (e) { return e[0]; }), datasets: [{ data: sorted.map(function (e) { return Math.round(e[1] * 1000) / 1000; }), backgroundColor: sorted.map(function (_, i) { return PALETTE[i % PALETTE.length] + 'cc'; }), borderColor: sorted.map(function (_, i) { return PALETTE[i % PALETTE.length]; }), borderWidth: 1 }] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { color: textColor, boxWidth: 12, padding: 10, font: { size: 11 } } }, tooltip: { callbacks: { label: function (c) { return ' ' + fmt(c.parsed) + ' credits'; } } } } }
      });
    }
    ...
    if (lineCtx) {
      ...
      if (days.length === 0) {
        ...
        charts['line'] = new Chart(lineCtx, {
          type: 'line',
          data: { labels: days, datasets: [{ label: 'Credits / day', data: days.map(function (d) { return Math.round(byDay[d] * 1000) / 1000; }), borderColor: PALETTE[0], backgroundColor: PALETTE[0] + '33', fill: true, tension: 0.3, pointRadius: days.length > 30 ? 0 : 3 }] },
          options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { label: function (c) { return ' ' + fmt(c.parsed.y) + ' credits'; } } } }, scales: { x: { ticks: { color: textColor, maxTicksLimit: 12 }, grid: { color: gridColor } }, y: { ticks: { color: textColor }, grid: { color: gridColor }, beginAtZero: true } } }
        });
      }
    }
  }
